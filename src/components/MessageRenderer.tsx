...
const MessageRenderer: React.FC<MessageRendererProps> = ({ content, language }) => {
  ...
  const formatText = (text: string) => {
    return text
      ...
      .map((line, lineIndex) => {
        ...
        // Enhanced title detection for better organization
        const isTitle = 
          ...
          (trimmedLine.endsWith(":") && (
            ...
            trimmedLine.startsWith("أين يمكن") ||
            trimmedLine.startsWith("متى يمكن") ||
            trimmedLine.startsWith("لماذا") ||
            ...
            (language === 'en' && trimmedLine.length > 30)
          ));
        ...
        // List formatting with fixed English alignment
        if (line.match(/^[\s]*[-*+]\s/)) {
        ...
        // Numbered lists with fixed English alignment
        if (line.match(/^[\s]*\d+\.\s/)) {
        ...
        if (line.includes("`") && !line.startsWith("```")) {
          ...
          while ((match = inlineCodeRegex.exec(line)) !== null) {
            if (match.index > lastIndex) {
              ...
              parts.push(formatInlineElements(textBefore));
            }
            parts.push(
              <code
          ...
          if (lastIndex < line.length) {
            parts.push(formatInlineElements(line.slice(lastIndex)));
          }
